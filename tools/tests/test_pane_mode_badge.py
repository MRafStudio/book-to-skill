...
# Владелец: «осталось только отцентрировать внутри плашки текст - по вертикали - ну поднять
# текст на 1 пиксель выше». Базовый `py-0.5` даёт 2 px сверху и снизу (текст стоит ровно по
# центру строки, а глаз ловит его ниже из-за метрик шрифта) - асимметрия 1 сверху / 3 снизу
# поднимает строку на 1 px, высота плашки при этом не меняется.
check("текст в плашке «нет связи с ядром» поднят на 1 px (paddingTop 1 / paddingBottom 3)",
      "paddingTop: 1" in nolink and "paddingBottom: 3" in nolink,
      "владелец: «поднять текст на 1 пиксель выше»; без асимметрии отступов текст снова сядет ниже центра")
