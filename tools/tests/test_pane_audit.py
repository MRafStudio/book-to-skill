...
2. размер обеих служебных кнопок шапки - тот же токен `icon-xs` (24×24), что у 📂/📋
   в блоке 1, а подсказка живёт на подписи (SDK-кнопка `title` в DOM не отдаёт);
3. в окне видно текущую категорию и предупреждение, что граф строится ТОЛЬКО по ней;
4. два действия: «Проверить» (чтение) и «Обновить связи» (запись related_skills);
5. ядро: маршрут `/audit_links`, CLI `audit-links`, загрузка модуля `tools/audit/links.py`;
6. живая проверка ядра: граф считается, `--apply` переписывает `related_skills`
...
def main() -> int:
    ...
    check("кнопка аудита есть в шапке и несёт подсказку",
          "cutSpan('🔗', GLYPH_CLS, TIP.auditLinks)" in src)
    ...
    check("связи ЛЕВЕЕ ведра, ведро - крайняя правая кнопка",
          src.index("cutSpan('🔗', GLYPH_CLS, TIP.auditLinks)")
          < src.index("cutSpan('🗑️', GLYPH_CLS, TIP.purgeAll)"))
    ...
    # 1б. ширина: шапка берёт ТОТ ЖЕ токен размера, что кнопки блока 1 (📂/📋).
    # Паритет проверяем не глазами, а токенами: у обеих групп кнопок размер должен
    # быть один и тот же, иначе «такая же ширина» держится до первой правки SDK.
    i_glyphs = src.find("Пиктограммы вместо подписей")
    b1 = src[i_glyphs:i_glyphs + 1500] if i_glyphs > 0 else ""
    n_b1 = b1.count("size: 'icon-xs'")
    check("кнопки блока 1 (📂/📋) помечены размером icon-xs",
          n_b1 == 2, f"найдено {n_b1} из 2")
    i_hdr = src.find("Правый верхний угол")
    hdr = src[i_hdr:i_hdr + 1500] if i_hdr > 0 else ""
    n_hdr = hdr.count("size: 'icon-xs'")
    check("обе служебные кнопки шапки помечены тем же размером icon-xs",
          n_hdr == 2, f"найдено {n_hdr} из 2")
    check("в шапке нет остатков «широкой плашки» (CHIP_FIT / justify-end / h-6)",
          "CHIP_FIT" not in hdr and "justify-end" not in hdr and "h-6" not in hdr,
          "иначе ширина кнопок шапки снова разойдётся с блоком 1")
    check("кегль эмодзи в шапке задан тем же классом, что у 📂/📋 (13 px)",
          "GLYPH_CLS" in hdr and "text-[0.8125rem]" in src)

    # 2. окно: категория + предупреждение
    i_open = src.find("Окно аудита связей")
    window = src[i_open:i_open + 7000] if i_open > 0 else ""
    # 7000, а не 4000: в окне теперь три абзаца (категория, порядок «сначала вложения» и
    # предупреждение о границе) - прежний срез обрезал хвост предупреждения.
    check("в окне названа текущая категория",
          "children: 'Категория: ' + ((cat || '').trim()" in window)
    check("в окне сказано, что граф строится ТОЛЬКО по выбранной категории",
          "Граф строится ТОЛЬКО по выбранной категории" in window)
    check("в окне сказано, что скиллы не удаляются и не переименовываются",
          "по выбранной категории" in window and "не удаляются и не переименовываются" in window)
    check("в окне назван порядок проверок: сначала вложения, потом граф",
          "СНАЧАЛА вложения" in window)
    check("в окне обещано, что «Обновить» дополнит недостающие разделы вложений",
          "дополнит недостающие разделы вложений" in window)
