...
def main() -> int:
    ...
    i_b4 = src.find("        n: 4,")
    i_b5 = src.find("        n: 5,")
    blk4 = src[i_b4:i_b5] if 0 < i_b4 < i_b5 else ""
    ...
    check("блок стоит в шаге 4 мастера (рядом с кнопкой «Сделать черновик»)",
          bool(blk4) and "draftBlock" in blk4 and "sendIntent('draft')" in blk4,
          f"block4@{i_b4}, len={len(blk4)}")
    ...
    # 5a-ter) Единый стандарт «окна»: грип и потолок как у поля в блоке 2. Владелец:
    # «добавить такие же Grip как у текстового поля в блоке 4 — и всё станет
    # стандартизированным». Грип живёт в ZONE_CAP, поэтому его получают ВСЕ окна.
    check("ZONE_CAP даёт грип и потолок 288 px (шаблон всех окон панели)",
          "resize: 'vertical'" in src and "maxHeight: 288," in src,
          "в ZONE_CAP нет resize/maxHeight — окна блока 3 останутся без грипа")
    check("у списка файлов есть подсказка про грип",
          "растянуть список файлов" in src)
    check("у раскладки по главам есть подсказка про грип",
    ...
    # План по главам построен по ПРЕЖНЕМУ черновику: и при смене источника, и при
    # новой генерации он обязан сниматься вместе с черновиком, иначе в спойлере
    # висят старые строки (владелец: «висит со старыми записями, даже при нажатии
    # Сгенерировать черновик»).
    check("план по главам снимается вместе с черновиком (смена источника)",
          "setDraft(null); setDraftText(null); setDraftFile(''); setInstalled(null)" in src
          and "setChapterPlan(null); setChapterOpen(false)" in src,
          "раскладка по главам не очищается вместе с черновиком")
    check("новая генерация черновика снимает старую раскладку по главам",
          src.count("setChapterPlan(null); setChapterOpen(false)") >= 2,
          "план по главам переживает «Сгенерировать черновик»")
    # «План по главам» - инструмент ДОЛИВА: он показывает пересечение тем с главами
    # УЖЕ СТОЯЩЕГО скилла. Пока имя свободно, пересекать не с чем (владелец: «следует
    # показывать только если в группе 3 выясняется, что скилл уже стоит»).
    check("«План по главам» показан только при доливе в существующий скилл",
          "const chapterSlot = !existing ? null : jsxs('div', {" in src,
          "кнопка раскладки показана и для нового скилла")
    check("раскладка привязана к целевому скиллу (сброс при смене имени)",
          "}, [wanted, targetTaken])" in src and "const targetTaken = !!existing" in src,
          "план по главам переживает смену имени скилла")
    ...
    check("при пропущенном шаге 2 подпись блока 3 говорит про markdown",
          "шаг 2 пропущен: источник уже markdown" in src,
          "человек не поймёт, почему анализа нет, а черновик нужен")
    # Ссылка на «шаг 2» допустима ТОЛЬКО в четырёх законных видах: рядом назван сам
    # шаг («Анализ источника»); прямо сказано, что он пропущен или не нужен; это
    # МЕТКА статуса («шаг 2 · сначала разбери источник»); либо источник заведомо НЕ
    ...
    check("ссылка на «шаг 2» всегда называет сам шаг или его пропуск",
          not bad_step2, f"голые ссылки: {[l.strip()[:70] for l in bad_step2[:3]]}")
