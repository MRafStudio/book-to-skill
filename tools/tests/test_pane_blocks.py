...
def main() -> int:
    ...
    check("красная граница блока 3 появляется только после прохода блока 2",
          "tone: (nameWarn && block2Passed) ? 'bad' : null," in src
    ...
    check("отпечаток входов разбора восстанавливается из /state при старте",
    ...
    check("блок 1 краснеет только за непройденный ШАГ 1 (источник не получен)",
          "rerunErrKind === 'source' ? 'bad' : null" in src,
          "блок 1 снова краснеет на любую ошибку")
    ...
    check("блок 1 называет своё: «не удалось получить источник»",
          "'не удалось получить источник: ' + rerunErr" in src)
