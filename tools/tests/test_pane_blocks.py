...
def main() -> int:
    ...
    check("«ДАЛЕЕ» блока 1 гаснет при пустом имени и называет причину",
          "disabled: !!busy || !trimSrc || nameWarn" in src and
          "nameWarn ? 'нужно имя скилла: пустым не поставим'" in src,
          "кнопка уйдёт вперёд без имени")
    check("пустое имя не проскакивает мимо кнопки (страховка в next1)",
