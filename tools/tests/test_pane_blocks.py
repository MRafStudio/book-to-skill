...
def main() -> int:
    ...
    # Блок 2: подсказка после разбора зовёт туда, куда ведёт АКТИВНАЯ «ДАЛЕЕ» - в блок 3
    # (категория и имя). Прежнюю формулировку агента («отчёт ниже - от прежних входов,
    # а не от этих») владелец прочитал как фигню: состояние отчёта несёт статус блока.
    check("подсказка блока 2 после разбора - дословно фраза владельца",
          ...
          "владелец: «Замени на - Следующим шагом укажи категорию и имя скилла»")
    ...
    # Состояние «отчёт от прежних входов» несут статус блока и шапка спойлера; подсказка
    # блока его больше НЕ повторяет - там фраза владельца про следующий шаг (см. выше).
    check("устаревший отчёт назван словами и в шапке блока, и в спойлере",
          "staleReport" in src and "'отчёт от прежних входов - прогони снова'" in src
          and "' · от прежних входов'" in src and "а не от этих" not in src)
