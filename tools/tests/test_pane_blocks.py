...
def main() -> int:
    ...
    check("молчащее ядро названо словами «нет связи с ядром» и красным",
          "нет связи с ядром" in src and "variant: 'bad'" in src,
          "плашка молчит о связи или остаётся зелёной при красном шарике")
