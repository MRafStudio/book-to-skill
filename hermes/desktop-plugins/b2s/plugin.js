...
function B2SPane({ ctx }) {
  ...
  /* Строка источника «как в поле»: и вотчер, и чтение сводки ходят в ядро через
     ...
     продолжает опрашивать прежний url и тянет в блок 3 чужой черновик (владелец:
      ...
      try {
        ...
        try {
          ...
          /* Черновик из /state НЕ берём: там черновик ПОСЛЕДНЕГО ПРОГОНА ядра, а
             ...
             url эти две вещи расходятся, и блок 3 показывал файлы прежней работы
  ...
  /* Ключ сменился — значит сменился ИСТОЧНИК. Всё, что показывал блок 3 (сводка и
     текст черновика) и блок 4 (план записи, статус установки), относилось к
