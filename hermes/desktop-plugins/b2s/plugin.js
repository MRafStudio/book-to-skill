 ...
 *
 *  Ссылок на голый «шаг 2» тут быть не может: блок 2 — это АНАЛИЗ источника, и при
 *  готовом markdown он пропускается, так что такая подпись отправляла человека
 *  в тупик. Назвать сам шаг («шаг 2 - «Анализ источника»») или сказать про пропуск
 *  можно - границу держит сторож `test_pane_draft.py`. */
function draftBitsOf({ draft, want, busy }) {
  ...
  if (!draft.has_draft) {
    ...
    bits.push('черновик делает кнопка ✎ в этом шаге')
    return bits
  }
...
function draftBrief(f) {
  ...
  push('')
  push('Закончив (именно в этом порядке):')
  push('1) проверь цельность: разделы источника покрыты, обрывков нет, ссылки на главы живые; всё, что нашёл, исправь ДО маркера;')
  push('2) маркер готовности - ПОСЛЕДНЕЕ действие: положи в каталог черновика READY.json с {"ready": true, "by": "Hermes Agent (BookToSkill)", "files": N, "chapters": N, "glossary_terms": M} (то же делает POST на /api/plugins/b2s/mark_ready). Без маркера панель считает черновик обрывком и не отпирает «ДАЛЕЕ»; маркер, поставленный до финальных проверок, отпирает её на середине работы - если после маркера всё же правил файлы, перезапиши маркер заново;')
...
function B2SPane({ ctx }) {
  ...
  /* Прошёл ли человек «ДАЛЕЕ» в шаге 3 («Категория и имя скилла»). До клика шаг
     нейтрален, после - зелёный: реквизиты не просто заполнены, по ним пошли дальше.
     Это второй флаг прохождения в панели и полная аналогия readySeen: цвет у шага
     красят РЕЗУЛЬТАТЫ (шаг 1 - проба источника, шаг 2 - разбор, шаг 4 - готовность
     черновика), а у шага 3 результата нет, поэтому его «пройдено» - сам клик.
     Владелец: «если в группе 3 нажать кнопку "ДАЛЕЕ" её граница не окрашивается
     зелёным, не смотря на то, что в ней ошибок не обнаружено и все требования
     исполнены» - до этого флага зелёный был недостижим: условие тона знало только
     'bad' и null. Правка имени и категории флаг НЕ гасит (реквизиты - не ключ
     черновика, панель пересобирает план на лету): красный от пустого или негодного
     имени перекрывает зелёный сам, а полный сброс панели флаг снимает. */
  const [nameSeen, setNameSeen] = useState(false)
  ...
  /* Занятость сессии задания в ref: вотчер `watchDraft` живёт в setInterval и берёт
     переменные замыканием на момент запуска - свежего стейта он не видит. Без этого
     панель объявляла работу законченной по первому же маркеру, хотя агент ещё
     проверял и правил черновик, и открывала «ДАЛЕЕ» на середине финальных дел. */
  const llmBusyRef = useRef(false)
  useEffect(() => {
    llmBusyRef.current = !!(llmSid && busyMap && busyMap[llmSid])
  }, [llmSid, busyMap])
  ...
  const resetAfterPurge = () => {
    ...
    setPreview(null); setReadySeen(false); setNameSeen(false); setDraftWait(false)
    ...
    const id = setInterval(async () => {
      ...
      try {
        ...
        if (out && out.has_draft) {
          ...
          /* Маркер + СВОБОДНЫЙ агент = работа действительно закончена. Пока агент
             занят в сессии задания, он ещё проверяет и правит черновик: «ДАЛЕЕ»
             остаётся запертой. Владелец видел обратное: «ты пишешь "финализирую...",
             а доступ к ДАЛЕЕ уже появился». */
          if (out.ready && !llmBusyRef.current) {
            done = true
            setDraftWait(false)
            setTone('done')
            const c = out.counts || {}
            ...
            const c = out.counts || {}
            setTone('working')
            saySrc('маркер готовности лежит' +
              (c.files ? ' (' + c.files + ' ' + plural(c.files, 'файл', 'файла', 'файлов') + ')' : '') +
              ', но агент ещё завершает работу - жду, пока он освободится')
          } else {
        ...
        } else if (!llmBusyRef.current) {
          /* Файлов нет, а агент уже свободен: задание доехало, но черновика не будет.
             Ждать нечего - иначе кнопка запуска осталась бы запертой до дедлайна. */
          done = true
          setDraftWait(false)
          setTone('error')
          saySrc('задание ушло, но каталога черновика в staging нет - смотри ответ агента в чате')
        }
  ...
  const runInstall = async (confirm) => {
    setBusy('install')
    setTone('working')
    saySrc(confirm ? 'установка · перенос в skills/…' : 'шаг 5 · предпросмотр переноса…')
    try {
      ...
      if (!confirm) {
        if (!out.has_skill_md) {
          setTone('error')
          saySrc((out.error || 'черновика в staging нет') + ' - сначала черновик в шаге 4')
        } else {
  ...
  const runPlan = async () => {
    ...
    setChapterBusy(true)
    setTone('working')
    saySrc('шаг 4 · раскладка по главам…')
    try {
  ...
  /* РЕГЛАМЕНТ ВЛАДЕЛЬЦА: поменялся источник (url или путь к файлу) - на шаге 2 не остаётся
     НИЧЕГО от прежнего разбора: ни метрик, ни пути к прежнему файлу, ни текста, ни зелёной
     «разобрано» в границе, ни подписи кнопки «Прогнать заново». Владелец: «поменяли в
     источнике адрес url или путь к файлу - на шаге 2 должна сброситься информация о
     результатах разбора, о предыдущем файле».
     Сравниваем со ПРЕДЫДУЩИМ вводом (ref), а не с отпечатком отчёта: сброс нужен ровно на
     СМЕНУ строки источника и не должен срабатывать при переоткрытии панели, где ввод
     восстановлен из storage, а отчёт ядра - от того же адреса. */
  const prevSrcRef = useRef(trimSrc)
  useEffect(() => {
    if (prevSrcRef.current === trimSrc) return
    prevSrcRef.current = trimSrc
    setReport(null)              // «последний прогон» ядра — не про этот ввод
    setFetchedSig('')            // разбор этим вводом не подтверждён
    setText(''); setTextInfo(null); setOutOpen(false)
    setRerunErr(''); setRerunErrKind('')
    setInstalled(null)
  }, [trimSrc])
  ...
  /* Страховка «разбор есть - текст показан». Живой случай: отчёт по адресу есть (граница
     шага 2 зелёная, кнопка «Прогнать заново»), а поле текста ПУСТОЕ - панель забыла
     подтянуть текст после разгона/переоткрытия. Владелец: «кнопка вещает "Прогнать заново",
     но в текстовом поле предварительного просмотра пусто бля!». Раз отчёт по этому вводу
     снят, текст обязан быть на экране; нет - читаем его по пути отчёта. */
  useEffect(() => {
    if (!analyzed || text || textBusy) return
    loadText(6000, (report && report.source_file) || '')
  }, [analyzed, text, textBusy])
    ...
    children: [
      ...
      text || textBusy
        ...
        : jsx('div', {
            ...
            children: 'текст не отдан ядром - нажми «Прогнать заново» в шаге 2, и он появится здесь'
          }),
    ...
    children: [
      ...
      ...(draft && draft.has_draft
        ? [
            ...
            jsx('div', {
              className: 'pt-1 opacity-70',
              children: 'черновик правят в чате; перегенерация - кнопка «✎» в шаге 4, а в skills/ переносит шаг 5'
            })
          ]
        : [
            jsx('div', {
  ...
  /* Работа по ЭТОМУ черновику ещё идёт: панель ждёт файлы после задания (`draftWait`),
     каталог уже появился без маркера (`draftWriting`) или агент занят в сессии, куда
     ушло задание (`busyMap` - тот же пульс, что в статусбаре).

     Маркера мало: агент кладёт `READY.json` и продолжает финальные дела (проверки,
     правки, пересчёт метаданных), поэтому запись в профиль открывается на «маркер ЕСТЬ
     и агент СВОБОДЕН». Владелец про это: «ты пишешь "финализирую..." - а ДАЛЕЕ уже
     доступна... надо выставлять флаг о завершении по факту ВСЕХ финальных дел». */
  const draftWork = !!(draftWait || llmTurn)
  const draftSettled = !!(draftReady && !draftWork)
  ...
  const next1 = async () => {
    ...
    if (!trimSrc) {
      setTone('error')
      say('шаг 1 · пустой источник: вставь URL или путь к файлу')
      return
    }
    ...
    if (mdSrc) {
      ...
      if (!analyzed) {
        ...
        if (!res.md) {
          onlyB(2)
          say('шаг 1 · источник не похож на markdown (стратегия ' + (res.strategy || '?') + ') - смотри шаг 2')
        }
      }
      return
    }
  ...
  const next2 = () => {
    /* Markdown-источнику разбор не нужен, но метрики для черновика всё же снимаются:
       идём тем же путём, что «ДАЛЕЕ» блока 1 (там прогон и обработка сбоя). Иначе шаг
       зелёный, а кнопка ведёт в никуда. */
    if (mdSrc && !analyzed) { next1(); return }
    if (!analyzed) {
      setTone('error')
      say('шаг 2 · сначала разбери источник - без отчёта не собрать реквизиты записи')
      return
    }
    onlyB(3)
  }
  ...
  const next3 = () => {
    ...
    if (!block2Passed) {
      setTone('error')
      say('шаг 3 · сначала пройди шаг 2 - «Анализ источника»: отчёт о разборе даёт имя скилла и метрики')
      return
    }
    if (nameWarn) {
      setTone('error')
      say('шаг 3 · без имени скилла нельзя: каталог установки называется именем')
      return
    }
    if (nameBad) {
      setTone('error')
      say('шаг 3 · такое имя Hermes не примет: только строчные латинские буквы, цифры, «-», «_», «.» (заглавные и пробелы валят шапку скилла)')
      return
    }
    dropPreview()   // реквизиты могли поменяться - прежний план записи устарел
    /* Шаг 3 пройден: только с этой отметкой его рамка зеленеет - «реквизиты заданы и
       человек по ним пошёл». Отметка стоит ПОСЛЕ проверок: клик, который никуда не
       пустил (пустое или негодное имя, не пройден шаг 2), зелёного не даёт. */
    setNameSeen(true)
    onlyB(4)
  }
  ...
  /* Шаг 4 → 5: записывать в профиль можно только ГОТОВЫЙ черновик.
    ...
    const next4 = () => {
      if (!block2Passed) {
        setTone('error')
        say('шаг 4 · сначала пройди шаг 2 - «Анализ источника»: без разбора черновик соберётся вслепую')
        return
      }
      if (!hasDraft) {
        setTone('error')
        say('шаг 4 · черновика в staging нет - нажми «Сделать черновик»')
        return
      }
      if (!draftSettled) {
        setTone('working')
        const c = (draft && draft.counts) || {}
        /* Два разных «рано»: маркера нет (пишется) и маркер есть, но агент ещё занят
           финальными делами. Второе раньше молча пропускало в блок 5. */
        say('шаг 4 · ' + (draftReady
          ? 'маркер готовности уже лежит, но агент ещё завершает работу над черновиком - кнопка записи откроется, когда он освободится'
          : 'черновик ещё пишется' +
            (c.files ? ' (' + c.files + ' ' + plural(c.files, 'файл', 'файла', 'файлов') +
              (c.chapters ? ', ' + c.chapters + ' ' + plural(c.chapters, 'глава', 'главы', 'глав') : '') + ')' : '') +
    ...
    children: [
                  ...
                  : core === null
                    ...
                    : core
                      /* Зелёный success, а не серый muted: «на связи» — это норма, и она должна
                         ...
                         Подпись - «ожидание задачи»: владелец просил именно её вместо
                         «локального режима». «Локальный режим» говорил про СПОСОБ работы,
                         а плашка описывает СОСТОЯНИЕ панели: ядро на связи, агент свободен,
                         панель ждёт задания. Хвост «прямой режим · локальный Python» владелец
                         отменил отдельно (что LLM сейчас не пишет, видно из первой ветки плашки). */
                      ? jsx(Badge, {
                          variant: 'success', style: BADGE_FIT, title: coreTip,
                          children: cutSpan('ожидание задачи')
                        })
                      : jsx(Badge, {
                          /* Владелец: «когда в плашке написано "нет связи с ядром" это
                             должно быть красноватым фоном плашки и оранжевыми буквами».
                             Раньше стоял вариант с именем `bad` - а его у Badge SDK НЕТ
                             (есть default|muted|success|warn|destructive|outline|solid), cva
                             молча не подставлял ничего: плашка выходила прозрачной с обычным
                             цветом текста. Красноватый фон даёт `destructive`
                             (bg-destructive/10), оранжевые буквы - классы варианта `warn`:
                             они уже в бандле приложения, а cn = twMerge, поэтому оранжевый
                             вытесняет text-destructive. «нет связи» - буквально, потому что
                             это и есть связь; цена и способ починки - в подсказке. */
                          variant: 'destructive',
                          /* Фон задаём ИНЛАЙНОМ, а не классом варианта: `bg-destructive/10`
                             - это 10% красного, на тёмной теме его почти не видно (владелец:
                             «Фон у плашки можно сделать красным? Я вижу только жёлтыми
      ...
      jsx(PaneBlock, {
        ...
        /* Красная граница — только когда источник уже на руках, а разбор не прошёл:
           ...
           А вот «пропущен» - это ПРОЙДЕННЫЙ шаг, и красить его серым нельзя: панель
           сама сказала «блок 2 пропустим» и тут же оставила шаг неотличимым от
           нетронутого. Владелец: «раз пропустим, то почему тогда блок 2 не окрасился
           зелёным? Ведь напрашивается? Он же уже валидный?» - зелёный тут честный:
           работы у шага не осталось, он не блокирует ни черновик, ни запись
           (`block2Passed = analyzed || mdSrc`). Почему именно пропущен, сказано в
           состоянии блока и в подсказке, так что путаницы с «разбор сделан» нет. */
        tone: mdSrc
          ? 'done'
        ...
        hint: mdSrc
          ...
          : (busy === 'rerun'
            ...
            : (rerunErr
              ? (rerunErrKind === 'source'
                ? 'источник не получен - причина и повтор в шаге 1'
        ...
        foot: jsx(NextBtn, {
          label: 'ДАЛЕЕ →',
          onClick: next2,
          /* Кнопка гасится там, где шаг НЕ пройден: для markdown-источника проходить
             нечего, а прежнее `!analyzed` держало её серой - вход в панель вёл в тупик
             («валидный источник, а шаг не пускает»). */
          disabled: !!busy || !block2Passed,
          fill: STEP_BG,
          title: mdSrc
            ? 'файл уже markdown - разбор не нужен: открыть следующий шаг'
            : (analyzed ? 'открыть черновик'
              : (staleReport
                ? 'входы изменились - прогони разбор заново'
                : (rerunErrKind === 'source' ? 'сначала получи источник в шаге 1' : 'сначала прогони анализ источника')))
        }),
        children: [
          mdSrc
          ...
          rerunErr
            ? jsx('div', Ell(rerunErrKind === 'source'
                ...
                ? 'источник не получен: ' + rerunErr + ' - повтор в шаге 1'
      ...
      jsx(PaneBlock, {
        ...
        /* Красная граница = «на этом шаге не хватает обязательного», и она уместна только
           когда шаг УЖЕ на своём месте, то есть блок 2 пройден. До этого имя пусто у всех
           (владелец: «блоки 1 и 2 ещё не выполнены, поэтому красная граница для блока 3
           это слишком вычурно - он должен быть без прохода блоков 1 и 2 тоже нейтральным»).
           Кнопка «ДАЛЕЕ» при этом всё равно погашена и называет причину - первой из них.
           Негодное имя (nameBad) краснит по тому же основанию: это незаполненный
           обязательный вход, только причина другая, и без него зелёный «шаг пройден»
           врал бы на имени, которое Hermes не примет.
           Зелёный - по флагу прохождения шага (nameSeen из next3), а не по заполненности
           полей: результат шага не «имя введено», а «по реквизитам пошли дальше».
           Обязательное условие обоих цветов - block2Passed: без шага 2 шаг 3 ни красный
           (владелец просил нейтральность до прохода шагов 1-2), ни зелёный. */
        tone: ((nameWarn || nameBad) && block2Passed)
          ? 'bad'
        ...
        hint: nameWarn
          ...
          : (existing
              ? 'имя занято: план ДОЛИВА соберётся на записи, замена - только твой явный выбор'
              : 'имя свободно: следующий шаг соберёт новый скилл'),
        foot: jsx(NextBtn, {
          label: 'ДАЛЕЕ →',
          onClick: next3,
          ...
          fill: STEP_BG,
          title: !block2Passed
            ? 'сначала пройди шаг 2 - «Анализ источника»: отчёт о разборе даёт имя скилла и его метрики'
            : (nameWarn
        ...
        children: [
          jsx('span', Ell('Это реквизиты ЗАПИСИ, а не входы разбора. Панель спрашивает их до черновика: шаг 4 (черновик) получает готовое имя и категорию, а план записи в шаге 5 сразу считается как долив или замена.',
            'text-[0.625rem] leading-snug text-(--ui-text-tertiary)')),
          jsxs('div', {
            className: 'grid grid-cols-2 gap-2',
            children: [
      ...
      jsx(PaneBlock, {
        ...
        state: draftSettled
          ? 'готов - в staging'
          : (draftReady
              ? '⏳ маркер есть, агент дописывает'
              : (draftWriting
                  ? '⏳ пишется - в профиль не пойдёт'
                  : (draftWait ? 'задание в чате - жду staging' : (busy === 'draft' ? 'пишется' : 'нет')))),
        /* Зелёным красит только «готов И агент свободен»: «каталог найден» и даже
           «маркер лежит» ещё не значат «работа закончена». */
        tone: draftSettled ? 'done' : (draftWork ? 'working' : null),
        open: !!openB[4],
        onToggle: () => toggleB(4),
        ...
        hint: draftSettled
          ...
          : (draftReady
              ? 'маркер готовности лежит, но агент ещё завершает работу - запись откроется, когда он освободится'
              : (draftWriting
                  ...
                  : (draftWait
                      ...
                      : (mdSrc
                          ? 'шаг 2 пропущен: источник уже markdown - жми «✎ Сделать черновик»'
        ...
        foot: jsx(NextBtn, {
          ...
          disabled: !!busy || !draftSettled || !block2Passed,
          fill: draftSettled ? STEP_BG : undefined,
          title: !block2Passed
            ? 'сначала пройди шаг 2 - «Анализ источника»: без разбора черновик соберётся по пустым метрикам'
            : (draftSettled
              ? 'открыть запись в профиль - имя и категорию собрали в шаге 3'
              : (draftReady
                  ? 'маркер готовности лежит, но агент ещё завершает работу над черновиком - дождись, пока он освободится'
                  : (draftWriting
            ...
            children: [
              jsx('div', {
                ...
                children: jsxs(Button, {
                  size: 'sm',
                  variant: 'ghost',
                  /* Пока по черновику идёт работа, второй запуск запрещён: две прозы
                     пишутся в один каталог и мешают друг другу. Раньше кнопка гасла
                     только при УЖЕ появившихся файлах - первые минуты, пока агент
                     читал источник, она оставалась живой и пускала вторую задачу.
                     Владелец: «нажимаем, запускается работа LLM, но кнопка всё равно
                     остаётся доступной». */
                  disabled: !!busy || draftWork,
                  onClick: () => sendIntent('draft'),
                  title: draftWork
                    ? (draftWriting
                        ? 'черновик ещё пишется - дождись маркера готовности'
                        : (draftReady
                            ? 'агент ещё завершает работу над черновиком - второй запуск запрещён'
                            : 'задание черновика уже в работе - второй запуск запрещён'))
                    : (draftSettled ? TIP.redraft : TIP.draft),
                  className: 'h-7 justify-start text-xs text-(--ui-text-primary)',
                  style: BTN_FIT,
                  children: [
                    jsx('span', { 'aria-hidden': true, style: { flexShrink: 0 }, children: '✎' }),
                    /* «Перегенерировать» - только про ГОТОВЫЙ черновик: на середине
                       генерации эта надпись обещала перегенерацию того, что ещё не
                       написано, и владелец читал её как «работа закончилась». */
                    cutSpan(draftSettled ? 'Перегенерировать с учётом замечаний' : 'Сделать черновик',
                      undefined,
                      draftSettled ? TIP.redraft : TIP.draft)
                  ]
                })
              ...
              jsx(Button, {
                ...
                disabled: !!busy || !draftSettled,
                onClick: () => sendIntent('review'),
                title: draftSettled
                  ? TIP.review
                  : (draftReady
                      ? 'агент ещё завершает работу над черновиком: критиковать рано'
                ...
                children: [
                  jsx('span', { 'aria-hidden': true, style: { flexShrink: 0 }, children: '🔍' }),
                  cutSpan('Критика и список правок', undefined, draftSettled ? TIP.review : (draftReady ? 'агент ещё завершает работу над черновиком: критиковать рано' : (draftWriting ? 'черновик ещё пишется: критиковать пока нечего' : TIP.reviewOff)))
                ]
              })
            ]
          }),
          jsx('span', Ell(draftSettled
            ? 'Правки к черновику и повторный прогон считаются заново: счёт растёт с числом итераций.'
            : (draftReady
                ? 'Маркер готовности лежит, но агент ещё завершает работу над черновиком: «Критика» и «ДАЛЕЕ» включатся, когда он освободится.'
                : (draftWriting
                    ? 'Черновик ещё пишется: «Критика» и «ДАЛЕЕ» включатся, когда LLM положит маркер готовности.'
                    : (draftWait
                        ? 'Задание ушло в чат: панель следит за staging - кнопки включатся, когда файлы появятся и агент закончит.'
                        : '«Критика» включится, когда в staging появится готовый черновик - его делает кнопка «✎» выше.'))),
            'pl-1 text-[0.625rem] leading-snug text-(--ui-text-tertiary)')),
          mdSrc && !hasDraft
            ? jsx('span', Ell('источник - готовый markdown: текст уже добыт шагом 1, анализ (шаг 2) не нужен, черновик можно делать сразу.',
                'pl-1 text-[0.625rem] leading-snug text-(--ui-text-tertiary)'))
            : null,
          draftBlock,
          chapterSlot,
      ...
      jsx(PaneBlock, {
        ...
        state: installed
          ...
          : (!draftSettled
              ? (draftReady ? '⏳ агент дописывает черновик'
                : (draftWriting ? '⏳ черновик ещё пишется' : 'черновика нет'))
              : (!readySeen ? 'пройди «ДАЛЕЕ» в шаге 4'
                : (preview
        ...
        foot: jsx(NextBtn, {
          ...
          disabled: !!busy || !draftSettled || !readySeen || !!installed || nameWarn || nameBad || !block2Passed,
          fill: (draftSettled && readySeen) ? STEP_BG : undefined,
          title: installed
            ? 'скилл уже записан в ' + installed.target + ' - чтобы поставить заново, измени черновик или входы'
            : (!block2Passed
              ? 'сначала пройди шаг 2 - «Анализ источника»: запись в профиль без разбора - это вслепую'
              : (nameWarn
                ? 'нужно имя скилла: пустым не поставим - каталог в skills/ должен быть назван'
                : (nameBad
                  ...
                  : (!draftSettled
                    ? (draftReady
                      ? 'агент ещё завершает работу над черновиком - запись откроется, когда он освободится'
                      : (draftWriting
                        ? 'черновик ещё пишется - записывать нечего, дождись маркера готовности'
                        : 'сначала сделай черновик - записывать нечего'))
                    : (!readySeen
                      ? 'сначала пройди «ДАЛЕЕ» в шаге 4: запись открывается только после готового черновика'
                      : (preview
        ...
        children: [
          jsx('span', Ell(installed
            ...
            : (!draftSettled
                ? (draftReady
                  ? 'маркер готовности есть, но агент ещё заканчивает работу над черновиком: запись откроется, когда он освободится'
                  : (draftWriting
                    ? 'черновик ещё пишется: ядро не пустит запись, пока LLM не положит маркер готовности (READY.json)'
                    : 'черновика нет: сначала кнопка «✎ Сделать черновик» в шаге 4'))
                : (!readySeen
                  ? 'черновик готов, но шаг 4 не пройден: жми «ДАЛЕЕ →» там - запись откроется после этого'
                  : (preview
                    ? (preview.mode === 'replace'
                    ...
                    : (existing
                      ...
                      : 'перенос в skills/ - сначала «Предпросмотр» без записи, пишет только «Установить». Любое действие в шагах 1-4 сбрасывает план')))),
            'text-[0.625rem] leading-snug text-(--ui-text-tertiary)')),
      ...
      /* ── Итог записи: «СОЗДАТЬ НОВЫЙ СКИЛЛ» ────────────────────────────────
         Владелец: «после того, как в шаге 5 на кнопке выводится "Успешно установлен", ниже
         5 группы и выше строки "REST: ..." должна появиться кнопка "СОЗДАТЬ НОВЫЙ СКИЛЛ":
         по клику шаг 5 сворачивается, шаг 1 разворачивается». Пока на кнопке шага 5
         «Предпросмотр» (плана нет) - этой кнопки быть не должно: скилл ещё не записан,
         начинать следующий рано. Условие ровно одно - ФАКТ установки (`installed`), а не
         «план собран»: после «Установить» человек и хочет начать заново. */
      installed
        ? jsx('div', {
            className: 'flex justify-end px-2 pb-1',
            children: jsx(NextBtn, {
              label: 'СОЗДАТЬ НОВЫЙ СКИЛЛ',
              /* Заливка та же, что у «ДАЛЕЕ» (STEP_BG): кнопка завершает цикл записи, и
                 владелец просил не выбивать её из темы - серый ghost среди зелёных кнопок
                 читался чужеродным. */
              fill: STEP_BG,
              title: 'скилл записан в ' + installed.target +
                ': вернёмся к шагу 1 и начнём следующий. Файлы в профиле остаются на месте, ' +
                'staging и сырьё не чистятся - это делает урна 🗑',
              onClick: () => {
                /* Гасим ФАКТ прежней записи и её план: они говорили про прошлые реквизиты,
                   и на новой работе шаг 5 показывал бы «установлен в <старый путь>». */
                setInstalled(null)
                setPreview(null)
                onlyB(1)
              }
