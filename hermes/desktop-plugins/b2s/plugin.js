                ...
                children: [
                  jsx(Input, {
                    ...
                    /* Пустое имя - не «просто пустое поле», а причина, по которой кнопка
                       ...
                       Цвет рамки различает степень: ОШИБКА (имя пустое или недопустимое)
                       - красная и такая же тонкая, как рамка блока 3 (`color-mix` 45%,
                       владелец: «такая же хорошая тоненькая линия»), ПРЕДУПРЕЖДЕНИЕ
                       (имя занято и подобное) - оранжевая, как было. Цвет ПОДПИСИ под
                       полем не трогаем: владелец просил оставить текст оранжевым. */
                    className: 'h-7 text-xs',
                    style: (nameWarn || nameBad)
                      ? { border: '1px solid ' + (nameBad
                          ? 'color-mix(in srgb, ' + STOP_RED + ' 45%, transparent)'
                          : WARN_YELLOW) }
                      : undefined
                  }),
