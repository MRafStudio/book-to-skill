...
-------------------------------------------------
    POST /probe               доступен ли источник (файл/URL) - без разбора
...
class ProbeBody(BaseModel):
    """Строка источника: доступен ли он (файл/URL) - проверка ДО разбора."""
    src: str = ""
...
def probe(body: ProbeBody) -> Dict[str, Any]:
    """Доступен ли источник: файл лежит на диске, URL отвечает. Без разбора и LLM.

    Критерий шага 1 целиком («Источник»): панель зовёт это при вводе и красит
    границу блока 1 по ответу, а не по чужому удачному разбору.
    """
    return core().do_probe(body.src)
